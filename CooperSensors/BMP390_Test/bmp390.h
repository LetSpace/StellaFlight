...
#define BMP_SENSOR_ID_VAL       0x60 // Default value in the datasheet
...
bool bmp_reset(bmp_t* bmp) {
    ...
    gpio_put(bmp->spi.cs, 0);
    spi_write_blocking(bmp->spi.inst, data_buffer, 2);
    gpio_put(bmp->spi.cs, 1);
...
bool bmp_check_chip_id(bmp_t* bmp) {
    uint8_t chip_id_reg = BMP_SENSOR_ID_REG | 0x80;
    uint8_t chip_id_val[2];

    //i2c_write_blocking(bmp->i2c.inst, bmp->i2c.addr, &chip_id_reg, 1, true);
    gpio_put(bmp->spi.cs, 0);
    spi_write_blocking(bmp->spi.inst, &chip_id_reg, 1);
    //gpio_put(bmp->spi.cs, 1);
    //i2c_read_blocking(bmp->i2c.inst, bmp->i2c.addr, chip_id_val, 1, false);
    //gpio_put(bmp->spi.cs, 0);
    spi_read_blocking(bmp->spi.inst, 0, chip_id_val, 2);
    gpio_put(bmp->spi.cs, 1);

    if (chip_id_val[1] != BMP_SENSOR_ID_VAL) {
#ifdef DEBUG
        printf("Returned Chip ID: 0x%02x\n", chip_id_val[0]);
        printf("Check your SPI configuration and connection.\n");
#endif
        return false;
    }
...
bool bmp_set_oversampling_rate(bmp_t* bmp) {
    uint8_t data_buffer[] = {
        BMP_OSR_REG, 
        (uint8_t)((bmp->oss << 3) | (bmp->oss << 0) & 0xFF)
    };
...
bool bmp_get_calib_coeffs(bmp_t* bmp) {
    uint8_t calib_coeffs_reg = BMP_CAL_REG | 0x80;
    uint8_t calib_coeffs_val[BMP_CAL_LEN];

    //i2c_write_blocking(bmp->i2c.inst, bmp->i2c.addr, &calib_coeffs_reg, 1, true);
    gpio_put(bmp->spi.cs, 0);
    spi_write_blocking(bmp->spi.inst, &calib_coeffs_reg, 1);
    //i2c_read_blocking(bmp->i2c.inst, bmp->i2c.addr, calib_coeffs_val, BMP_CAL_LEN, false);
    spi_read_blocking(bmp->spi.inst, 0, calib_coeffs_val, 1);
    spi_read_blocking(bmp->spi.inst, 0, calib_coeffs_val, BMP_CAL_LEN);
    gpio_put(bmp->spi.cs, 1);
...
bool bmp_read_uncalibrated_temperature(bmp_t* bmp) {
    uint8_t temp_reg = BMP_TEMPERATURE_REG | 0x80;
    uint8_t temp_val[3];

    //i2c_write_blocking(bmp->i2c.inst, bmp->i2c.addr, &temp_reg, 1, true);
    gpio_put(bmp->spi.cs, 0);
    spi_write_blocking(bmp->spi.inst, &temp_reg, 1);
    //i2c_read_blocking(bmp->i2c.inst, bmp->i2c.addr, temp_val, 3, false);
    spi_read_blocking(bmp->spi.inst, 0, temp_val, 1);
    spi_read_blocking(bmp->spi.inst, 0, temp_val, 3);
    gpio_put(bmp->spi.cs, 1);
...
bool bmp_read_uncalibrated_pressure(bmp_t* bmp) {
    uint8_t pres_reg = BMP_PRESSURE_REG | 0x80;
    uint8_t pres_val[3];

    //i2c_write_blocking(bmp->i2c.inst, bmp->i2c.addr, &pres_reg, 1, true);
    gpio_put(bmp->spi.cs, 0);
    spi_write_blocking(bmp->spi.inst, &pres_reg, 1);
    //i2c_read_blocking(bmp->i2c.inst, bmp->i2c.addr, pres_val, 3, false);
    spi_read_blocking(bmp->spi.inst, 0, pres_val, 1);
    spi_read_blocking(bmp->spi.inst, 0, pres_val, 3);
    gpio_put(bmp->spi.cs, 1);
...
bool bmp_get_pressure_temperature(bmp_t* bmp) {
    ...
    
    {
        ...
        while ((status & 0x60) != 0x60) {
            uint8_t status_reg = BMP_STATUS_REG | 0x80;
            //i2c_write_blocking(bmp->i2c.inst, bmp->i2c.addr, &status_reg, 1, true);
            gpio_put(bmp->spi.cs, 0);
            spi_write_blocking(bmp->spi.inst, &status_reg, 1);
            //i2c_read_blocking(bmp->i2c.inst, bmp->i2c.addr, &status, 1, false);
            spi_read_blocking(bmp->spi.inst, 0, &status, 1);
            spi_read_blocking(bmp->spi.inst, 0, &status, 1);
            gpio_put(bmp->spi.cs, 1);
            sleep_ms(1);
            //printf("\nstatus register: %d", status);
        }
    }
...
bool bmp_init(bmp_t* bmp) {
    ...
    spi_init(bmp->spi.inst, 100000);
    gpio_set_function(bmp->spi.sck, GPIO_FUNC_SPI);
    gpio_set_function(bmp->spi.mosi, GPIO_FUNC_SPI);
    gpio_set_function(bmp->spi.miso, GPIO_FUNC_SPI);
    ...
    gpio_init(bmp->spi.cs);
    gpio_set_dir(bmp->spi.cs, GPIO_OUT);
    gpio_put(bmp->spi.cs, 1);
